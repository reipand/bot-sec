...
export const data = new SlashCommandBuilder()
  .setName("verify-all")
  .setDescription("Give the verified role to all non-bot members (batch processed)")
...
export async function execute(interaction) {
  ...
  const targets = [
    ...members.filter((m) => !m.roles.cache.has(verifiedRole.id) && !m.user.bot).values(),
  ];
  ...
  for (let i = 0; i < targets.length; i += BATCH_SIZE) {
    await Promise.all(
      targets.slice(i, i + BATCH_SIZE).map(async (m) => {
        try {
          await m.roles.add(verifiedRole, `/verify-all by ${interaction.user.tag}`);
          success++;
        } catch {
          failed++;
        }
  ...
  await logger.log(interaction.guild, {
    title: "Verify-All Executed",
    color: 0x00cc66,
    fields: [
      { name: "Moderator", value: `${interaction.user.tag} (${interaction.user.id})` },
      { name: "Verified", value: String(success), inline: true },
      { name: "Failed", value: String(failed), inline: true },
    ],
  });

  await interaction.editReply({
    content: `Done. Verified **${success}** member(s).${failed ? ` (${failed} failed)` : ""}`,
  });
}
